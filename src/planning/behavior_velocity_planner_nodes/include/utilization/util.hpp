  ...
  const autoware_auto_planning_msgs::msg::TrajectoryPoint & p)
{
  ...
  point.set__y(p.pose.position.y);
...
inline geometry_msgs::msg::Pose getPose(
  const autoware_auto_planning_msgs::msg::PathWithLaneId & path, size_t idx)
{
  return path.points.at(idx).point.pose;
}

inline geometry_msgs::msg::Pose getPose(
  const autoware_auto_planning_msgs::msg::Trajectory & traj, size_t idx)
{
