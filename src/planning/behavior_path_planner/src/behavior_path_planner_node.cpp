...
: Node("behavior_path_planner", node_options)
{
  ...
  velocity_subscriber_ = create_subscription<VehicleKinematicState>(
  ...
  hazard_signal_publisher_ = create_publisher<HazardLightsCommand>("~/output/hazard_light_cmd", 1);
  ...
  debug_path_publisher_ = create_publisher<nav_msgs::msg::Path>("~/debug/path_for_visualize", 1);
...
void BehaviorPathPlannerNode::requestOsmBinaryMap()
{
...
BehaviorPathPlannerParameters BehaviorPathPlannerNode::getCommonParam()
{
  ...
  const auto vehicle_info =
    autoware::common::vehicle_constants_manager::declare_and_get_vehicle_constants(*this);
  p.vehicle_width = vehicle_info.vehicle_width;
  p.vehicle_length = vehicle_info.vehicle_length;
  p.wheel_tread = vehicle_info.wheel_tread;
  p.wheel_base = vehicle_info.wheel_base;
  p.front_overhang = vehicle_info.overhang_front;
  p.rear_overhang = vehicle_info.overhang_rear;
  p.left_over_hang = vehicle_info.overhang_left;
  p.right_over_hang = vehicle_info.overhang_right;
  p.base_link2front = vehicle_info.offset_longitudinal_max;
...
void BehaviorPathPlannerNode::run()
{
  ...
  debug_path_publisher_->publish(util::toNavPath(clipped_path));
  ...
  hazard_lights_command.stamp = turn_signal.header.stamp;

  turn_indicators_command.command = turn_indicators_command.DISABLE;
  hazard_lights_command.command = hazard_lights_command.DISABLE;

  switch (turn_signal.data) {
    case turn_signal.NONE:
      turn_indicators_command.command = turn_indicators_command.NO_COMMAND;
      hazard_lights_command.command = hazard_lights_command.NO_COMMAND;
      break;
    case turn_signal.LEFT:
      turn_indicators_command.command = turn_indicators_command.ENABLE_LEFT;
      break;
    case turn_signal.RIGHT:
      turn_indicators_command.command = turn_indicators_command.ENABLE_RIGHT;
      break;
    case turn_signal.HAZARD:
      hazard_lights_command.command = hazard_lights_command.ENABLE;
      break;
    default:
      turn_indicators_command.command = turn_indicators_command.DISABLE;
      hazard_lights_command.command = hazard_lights_command.DISABLE;
  ...
  turn_signal_publisher_->publish(turn_indicators_command);
  hazard_signal_publisher_->publish(hazard_lights_command);
...
void BehaviorPathPlannerNode::onVelocity(const VehicleKinematicState::ConstSharedPtr msg)
{
  std::shared_ptr<TwistStamped> twist_stamped{std::make_shared<TwistStamped>()};
  twist_stamped->header.stamp = msg->header.stamp;
  twist_stamped->twist.linear.x = msg->state.longitudinal_velocity_mps;
  twist_stamped->twist.linear.y = msg->state.lateral_velocity_mps;
  twist_stamped->twist.linear.z = 0.0f;
  planner_data_->self_velocity = twist_stamped;
}
void BehaviorPathPlannerNode::onPerception(const PredictedObjectsOrgMsg::ConstSharedPtr msg)
{
  ...
  for (auto const & obj:msg->objects) {
    ...
    predicted_object.existence_probability = obj.existence_probability;
    predicted_object.classification = obj.classification;
    for (auto const & shape : obj.shape) {
    ...
    for (auto const & predicted_path_org :obj.kinematics.predicted_paths) {
      ...
      for (size_t i = 0; i < predicted_path_org.path.size(); i++) {
        auto const & pose = predicted_path_org.path.at(i);
        ...
        pose_wcs.header.stamp = pose_time_calculated;

        predicted_path.path.push_back(pose_wcs);
      }
  ...
  planner_data_->dynamic_object = predicted_objects;
}
void BehaviorPathPlannerNode::onExternalApproval(const ApprovalMsg::ConstSharedPtr msg)
{
  ...
  planner_data_->approval.is_approved.stamp = msg->stamp;
}
