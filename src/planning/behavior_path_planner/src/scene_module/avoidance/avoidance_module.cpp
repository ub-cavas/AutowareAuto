...
AvoidancePlanningData AvoidanceModule::calcAvoidancePlanningData(DebugData & debug) const
{
  ...
  data.arclength_from_ego = util::calcPathArcLengthArray(
    data.reference_path,
    0,
    data.reference_path.points.size(),
  ...
  DebugData & debug) const
{
  ...
  const auto expanded_lanelets = lanelet::utils::getExpandedLanelets(
    current_lanes,
    parameters_.detection_area_left_expand_dist,
  ...
  for (const auto & i : lane_filtered_objects_index) {
    ...
    const auto & object_pos = object.kinematics.initial_pose.pose.position;
...
void AvoidanceModule::updateRegisteredRawShiftPoints()
{
  ...
  DEBUG_PRINT(
    ...
    avoidance_data_.ego_closest_path_index,
    registered_raw_shift_points_.size(),
    avoid_points.size());
  ...
  for (auto & o : objects) {
    ...
    if (!has_enough_distance) {
      ...
      if (required_jerk > parameters_.max_lateral_jerk) {
        DEBUG_PRINT(
          "object is ignored required_jerk is too large (req: %f, max: %f)",
          required_jerk,
    ...
    DEBUG_PRINT(
      ...
      ap_avoid.end_longitudinal,
      ap_return.end_longitudinal,
      nominal_avoid_distance,
      avoiding_distance,
      avoid_margin,
    ...
    const auto has_large_length_change = [&]() {
        for (const auto & original : being_merged_points) {
          ...
          DEBUG_PRINT(
            ...
            original.length,
            original.end_longitudinal,
            combined_ap.start_longitudinal,
            combined_ap.getGradient(),
            new_length,
            has_large_change);
...
void AvoidanceModule::trimSharpReturn(AvoidPointArray & shift_points) const
{
  ...
  for (size_t i = 0; i < shift_points_orig.size(); ++i) {
    ...
    if (!isNegative(sp_now)) {
      shift_points.push_back(sp_now);
      DEBUG_PRINT(
        "i = %lu, positive shift. take this one. sp_now.length * sp_now.start_length = %f",
        i,
  ...
  AvoidPointArray & sp_candidates, AvoidPointArray & current_raw_shift_points) const
{
  ...
  ShiftPoint last_sp;  // the return-shift will be generated after the last shist point.
  {
    ...
    if (!has_candidate_point && has_registered_point) {
      last_sp = fillAdditionalInfo(
        AvoidPoint{static_cast<AvoidPoint &>(path_shifter_.getLastShiftPoint().get())});
    }
    ...
    if (has_candidate_point && has_registered_point) {
      ...
      const auto & sp = fillAdditionalInfo(
        AvoidPoint{static_cast<AvoidPoint &>(path_shifter_.getLastShiftPoint().get())});
  ...
  if (remaining_distance < prepare_distance_scaled + avoid_distance_scaled) {
    ...
    DEBUG_PRINT(
      ...
      last_sp_distance,
      nominal_prepare_distance,
      nominal_avoid_distance,
      remaining_distance,
      variable_prepare_distance,
      scale,
      prepare_distance_scaled,
      avoid_distance_scaled);
  } else {
...
void AvoidanceModule::modifyPathVelocityToPreventAccelerationOnAvoidance(ShiftedPath & path) const
{
  ...
  DEBUG_PRINT(
    "s: %f, t: %f, v0: %f, a: %f, vmax: %f, ego_i: %lu, target_i: %lu",
    s,
    t,
    v0,
    parameters_.max_avoidance_acceleration,
    vmax,
    ego_idx,
    target_idx);
}
  ...
  for (const auto & p : avoidance_data_.reference_path.points) {
    for (const auto & id : p.lane_ids) {
      const lanelet::ConstLanelet ll =
        planner_data_->route_handler->getLaneletsFromId(static_cast<int64_t>(id));
  ...
  const AvoidPointArray & candidates, const PathShifter & shifter) const
{
  ...
  for (size_t i = 0; i < candidates.size(); ++i) {
    ...
    if (std::abs(candidate.length - current_shift) > NEW_POINT_THRESHOLD) {
      DEBUG_PRINT(
        "%s, New shift point is found!!! shift change: %f -> %f",
        pfx,
        current_shift,
        candidate.length);
...
{
  return std::to_string(obj.object.object_id) + std::to_string(obj.object.object_id) +
         std::to_string(obj.object.object_id);
}
...
void AvoidanceModule::updateRegisteredObject(const ObjectDataArray & now_objects)
{
  const auto updateIfDetectedNow = [&now_objects, this](auto & registered_object) {
      ...
      const auto r_pos = registered_object.object.kinematics.initial_pose.pose;
      const auto similar_pos_obj = std::find_if(
        n.begin(), n.end(), [&](const auto & o) {
          return calcDistance2d(r_pos, o.object.kinematics.initial_pose.pose) < POS_THR;
        });
...
bool AvoidanceModule::isTargetObjectType(const PredictedObject & object) const
{
  ...
  const auto t = object.classification.front().classification;
  ...
  if (!path.shift_length.empty()) {
    if (isAvoidancePlanRunning()) {
      ...
      if (diff > tl_on_threshold) {
        turn_signal.turn_signal.data = TurnSignalDecider::TurnSignal::LEFT;
      } else if (diff < -tl_on_threshold) {
        turn_signal.turn_signal.data = TurnSignalDecider::TurnSignal::RIGHT;
      }
    }
  }
