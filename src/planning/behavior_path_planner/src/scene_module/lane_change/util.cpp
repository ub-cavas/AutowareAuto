      ...
      for (size_t i = 0; i < shifted_path.path.points.size(); ++i) {
        ...
        if (i < *lanechange_end_idx) {
          point.lane_ids.insert(
            point.lane_ids.end(),
            reference_path1.points.back().lane_ids.begin(),
            reference_path1.points.back().lane_ids.end());
          point.lane_ids.insert(
            point.lane_ids.end(),
            reference_path2.points.front().lane_ids.begin(),
...
std::vector<LaneChangePath> selectValidPaths(
  const std::vector<LaneChangePath> & paths,
...
bool selectSafePath(
  ...
  const LaneChangeParameters & ros_parameters,
  ...
  for (const auto & i : current_lane_object_indices) {
    ...
    for (const auto & obj_path : predicted_paths) {
      ...
      if (isObjectFront(current_pose, obj.kinematics.initial_pose.pose)) {
        ...
        thresh = util::l2Norm(obj.kinematics.initial_twist.twist.linear) * stop_time;
      }
  ...
  for (const auto & i : target_lane_object_indices) {
    ...
    if (ros_parameters.use_predicted_path_outside_lanelet) {
      ...
      for (const auto & llt : target_lanes) {
        if (lanelet::utils::isInLanelet(obj.kinematics.initial_pose.pose, llt)) {
          is_object_in_target = true;
        }
      }
    }
