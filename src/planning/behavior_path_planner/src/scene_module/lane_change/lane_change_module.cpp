...
TurnSignalInfo LaneChangeModule::getTurnSignalAndDistance(const PathWithLaneId & path) const
{
  ...
  for (size_t idx = 1; idx < path.points.size(); ++idx) {
    ...
    std::vector<int64_t> prev_lane_ids_i64(prev_lane_ids.begin(), prev_lane_ids.end());
    const auto prev_lanes = route_handler->getLaneletsFromIds(prev_lane_ids_i64);

    std::vector<int64_t> lane_ids(path_point.lane_ids.begin(), path_point.lane_ids.end());
    const auto lanes = route_handler->getLaneletsFromIds(lane_ids);
...
bool LaneChangeModule::isLaneBlocked(const lanelet::ConstLanelets & lanes) const
{
  ...
  for (const auto & obj : planner_data_->dynamic_object->objects) {
    if (
      obj.classification.front().classification == ObjectClassification::CAR ||
      obj.classification.front().classification == ObjectClassification::TRUCK ||
      obj.classification.front().classification == ObjectClassification::BUS ||
      obj.classification.front().classification == ObjectClassification::MOTORCYCLE)
    {
      const auto velocity = util::l2Norm(obj.kinematics.initial_twist.twist.linear);
      if (velocity < static_obj_velocity_thresh) {
        const auto position =
          lanelet::utils::conversion::toLaneletPoint(obj.kinematics.initial_pose.pose.position);
