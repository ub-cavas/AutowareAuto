...
// Extracts lane_ids from HADMAPSegment stored in Map primitives
std::vector<int64_t> extractLaneIds(const autoware_auto_mapping_msgs::msg::HADMapSegment & segment)
{
  std::vector<int64_t> lane_ids;
  for (auto const & primitive : segment.primitives) {
    lane_ids.push_back(primitive.id);
  }
  return lane_ids;
}
...
{
  const auto & route_segments = route_msg.segments;
  for (std::size_t i = 0; i < route_segments.size(); i++) {
    const auto & route_segment = route_segments.at(i);
    for (const auto & lane_id : extractLaneIds(route_segment)) {
      for (std::size_t j = i + 1; j < route_segments.size(); j++) {
        const auto & future_route_segment = route_segments.at(j);
        if (exists(extractLaneIds(future_route_segment), lane_id)) {
          return true;
        }
      }
    }
...
void RouteHandler::setRouteLanelets()
{
  ...
  for (const auto & route_segment : route_msg_.segments) {
    for (const auto & id : extractLaneIds(route_segment)) {
      const auto & llt = lanelet_map_ptr_->laneletLayer.get(id);
      route_lanelets_.push_back(llt);
      if (id == route_segment.preferred_primitive_id) {
  ...
  if (!route_msg_.segments.empty()) {
    for (const auto & id : extractLaneIds(route_msg_.segments.back())) {
      const auto & llt = lanelet_map_ptr_->laneletLayer.get(id);
      goal_lanelets_.push_back(llt);
    }
    for (const auto & id : extractLaneIds(route_msg_.segments.front())) {
      const auto & llt = lanelet_map_ptr_->laneletLayer.get(id);
...
bool RouteHandler::isInGoalRouteSection(const lanelet::ConstLanelet & lanelet) const
{
  if (route_msg_.segments.empty()) {
    ...
    return exists(extractLaneIds(route_msg_.segments.back()), lanelet.id());
  }
}
