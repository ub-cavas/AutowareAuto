...
sensor_msgs::msg::PointCloud2 autoware::mapping::point_cloud_mapping::make_pc(
  const std::vector<CloudModifier::value_type> & pts,
  const std::string & frame)
{
  sensor_msgs::msg::PointCloud2 pc;
  CloudModifier modifier{pc, frame};
...
sensor_msgs::msg::PointCloud2 autoware::mapping::point_cloud_mapping::make_pc(
  ...
  const std::string & frame)
{
  std::vector<CloudModifier::value_type> pts(size);
  auto idx = 0U;
  std::generate(
    pts.begin(), pts.end(), [&idx, offset]() {
      auto val = static_cast<float_t>((idx++) + offset);
      return CloudModifier::value_type{val, val, val, val};
    });
...
std::vector<CloudModifier::value_type> autoware::mapping::point_cloud_mapping::get_cells(
