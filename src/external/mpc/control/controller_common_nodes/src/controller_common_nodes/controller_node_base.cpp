  ...
  const std::string & diagnostic_topic)
{
  ...
  m_state_sub = create_subscription<State>(state_topic, QoS{10},
      [this](const State::SharedPtr msg) {on_state(msg);}, SubAllocT{});
  m_trajectory_sub = create_subscription<Trajectory>(trajectory_topic, QoS{10},
  ...
  m_static_tf_sub = create_subscription<TFMessage>(static_tf_topic, QoS{10},
...
bool ControllerBaseNode::try_compute(const State & state)
{
  ...
  Diagnostic diag;
  if (m_diagnostic_pub) {
    diag.header.computation_start = time_utils::to_message(start);
  ...
  const auto diagnostic_fn = [this, start, &state, &diag]() -> void {
      if (m_diagnostic_pub) {
        diag.header.runtime = time_utils::to_message(std::chrono::system_clock::now() - start);
        compute_diagnostic(
          *m_controller,
          state,
