...
"""Launch nodes to produce obstacle bounding boxes using lidar data from the lgsvl simulator."""
...
def get_param_file(package_name, file_name):
    ...
    assumes the param file has the given file_name
    and is under "param" folder inside the given package_name.
    """
...
def generate_launch_description():
    ...
    euclidean_cluster_node_runner = launch_ros.actions.Node(
        ...
        parameters=[get_param_file('euclidean_cluster_nodes',
                                   'vlp16_sim_lexus_polygon_prisms.param.yaml')],
        remappings=[
    ...
    rviz_cfg_path = os.path.join(examples_pkg_path, 'rviz2',
                                 'lidar_polygon_prisms_lgsvl.rviz')
