...
void OSQPInterface::updateP(const Eigen::MatrixXd & P_new)
{
  ...
  */
  ...
  osqp_update_P(
    m_work.get(), P_csc.m_vals.data(), OSQP_NULL,
...
void OSQPInterface::updateA(const Eigen::MatrixXd & A_new)
{
  ...
  */
  ...
  osqp_update_A(
    m_work.get(), A_csc.m_vals.data(), OSQP_NULL,
  ...
  int64_t> OSQPInterface::solve()
{
  ...
  std::tuple<std::vector<float64_t>, std::vector<float64_t>, int64_t, int64_t, int64_t> result =
    std::make_tuple(
    sol_primal, sol_lagrange_multiplier, status_polish, status_solution,
    status_iteration);
