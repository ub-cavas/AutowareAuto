...
#include <planning/path_display.hpp>
...
: rviz_common::RosTopicDisplay<autoware_auto_planning_msgs::msg::PathWithLaneId>(),
...
void PathDisplay::onInitialize()
{
  ...
  topic_property_->setValue("path");
  topic_property_->setDescription("PathWithLaneId topic to subscribe to.");
}

void PathDisplay::load(const rviz_common::Config & config)
{
  Display::load(config);
  m_marker_common->load(config);
}

void PathDisplay::update(float32_t wall_dt, float32_t ros_dt)
{
...
void PathDisplay::updateProperty()
{
...
void PathDisplay::processMessage(const Path::ConstSharedPtr msg)
{
  msg_cache = msg;
  m_marker_common->clearMarkers();
  auto count = 0;
...
visualization_msgs::msg::Marker::SharedPtr PathDisplay::create_pose_marker(
...
visualization_msgs::msg::Marker::SharedPtr PathDisplay::create_velocity_marker(
  const PathPoint & point) const
{
  auto color = color_property_->getColor();
  ...
  marker->action = visualization_msgs::msg::Marker::ADD;
  marker->ns = "path_velocity";
  marker->pose.position = point.point.pose.position;
  marker->scale.z = 0.2f * text_scale;
  marker->color.a = text_alpha;
  marker->color.r = static_cast<float>(color.redF());
  marker->color.g = static_cast<float>(color.greenF());
  marker->color.b = static_cast<float>(color.blueF());
  marker->text = std::to_string(point.point.twist.linear.x) + "mps";
...
PLUGINLIB_EXPORT_CLASS(autoware::rviz_plugins::PathDisplay, rviz_common::Display)
